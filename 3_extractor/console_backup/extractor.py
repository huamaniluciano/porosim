...
═══════════════════════════════════════════════════════════════════════════
  ...
  python extractor.py                 → INTERACTIVE: choose the solution (with the
                                        file browser) and navigate the module menu.

  python extractor.py Solutions_....h5 [modulo] [--voltage V]
                                      → BATCH: runs a module on that solution
                                        with no menus. Without 'module' → lists the
                                        available ones. Iterable.

  python extractor.py list            → lists the available modules.

The extractor reads a SOLUTION from the solver (Pillar 2): the <...>.h5 and its
<...>_sim.json (in the same folder). Each module generates a plot and,
optionally, saves an image/table in that same folder.

Modules (each is a .py with procesar(ruta_solucion) in modulos/<category>/):
...
Requires FEniCS (dolfin 2019.1.0) + matplotlib + numpy. Batch accepts
answers to each module's prompts via stdin (voltage, saving).
"""
import os
import sys
...
# Este script vive en 3_extractor/console_backup/: la raíz del pilar es el
# padre, y la del repo (donde está RESULTS/) el abuelo.
AQUI            = pathlib.Path(__file__).resolve().parent
PILAR           = AQUI.parent
CARPETA_MODULOS = PILAR / "modulos"
CARPETA_RESUMEN = PILAR / "solution_summary"
SCRIPT_RESUMEN  = CARPETA_RESUMEN / "solution_summary.py"

# Historial e I/O viven en RESULTS/ (no en la carpeta del extractor).
_REPO_ROOT            = PILAR.parent
...
def _imprimir_modulos():
    ...
    print("\nExample:  python extractor.py <solution.h5> axis_profile_potential --voltage -1.0")
...
def _modo_batch(ruta_sol, nombre_modulo, voltaje):
    """Corre un módulo sin prompts ni ventanas, usando el contrato preparar/guardar."""
    ...
    if not getattr(modulo, "USA_VOLTAJE", True):
        ...
        datos, ctx = prep
        rutas = modulo.guardar(datos, ctx, ruta_solucion, con_datos=False)
    else:
        if voltaje is None:
            _err("Missing --voltage. Examples:  --voltage -1.0  |  --voltage all")
...
# =============================================================================
# MODO INTERACTIVO (menús + explorador, como el ejecutor original)
# =============================================================================
...
def seleccionar_opcion(opciones, tipo):
    if not opciones:
        print(f"\nNo hay {tipo}s available here.")
        return None
    print(f"\n=== Available {tipo}s ===")
    for i, item in enumerate(opciones, 1):
        print(f"[{i}] {item.name}")
    while True:
        sel = input(f"\nNumber of {tipo} (or 'q' to go back): ").strip()
        if sel.lower() == 'q':
            return None
        try:
            idx = int(sel) - 1
...
def _modo_interactivo():
    ...
    if not CARPETA_MODULOS.is_dir():
        _err(f"The 'modulos' folder was not found in {PILAR}.")

    # BUGFIX: En el modo consola interactivo, los módulos importan `matplotlib.pyplot`
    # al ser cargados, lo que inicializa PyQt. Si PyQt se inicializa ANTES que FEniCS
    # (MPI), UCX crashea horriblemente al cargar la malla. Forzamos la carga de FEniCS
    # ahora mismo para que MPI se apropie del proceso antes que Qt.
    if str(CARPETA_MODULOS) not in sys.path:
        sys.path.insert(0, str(CARPETA_MODULOS))
    import porosim_comun as pc
    pc._fenics()
...
if __name__ == "__main__":
    ...
    if "--voltage" in argv:
        ...
        if i + 1 >= len(argv):
            _err("--voltage needs a value:  --voltage -1.0")
        voltaje = argv[i + 1]
    ...
    if argv and argv[0] in ("list", "--list"):
        ...
        _modo_batch(argv[0], nombre_modulo, voltaje)
    elif argv:
        _err(f"Unrecognized argument: '{argv[0]}'.\n"
             f"   python extractor.py [solucion.h5 [modulo] [--voltage V]]\n"
             f"   python extractor.py list         (available modules)")
    else:
        _modo_interactivo()
